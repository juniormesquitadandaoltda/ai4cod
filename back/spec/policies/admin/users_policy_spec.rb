...
module ADMIN
  RSpec.describe UsersPolicy, type: :policy do
    ...
    describe '#personificate' do
      it 'must be successful' do
        expect(access_admin.send(:personificate)).to be_truthy
      end

      it 'must be failure' do
        expect(access_user.send(:personificate)).to be_falsey
        model.profile_admin!
        expect(access_admin.send(:personificate)).to be_falsey
      end
    end

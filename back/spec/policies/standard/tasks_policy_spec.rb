...
module STANDARD
  RSpec.describe TasksPolicy, type: :policy do
    ...
    let!(:access_user) { described_class.new(pundit_user.merge(user:, subscription: nil, client: user), nil) }
    let!(:access_admin) { described_class.new(pundit_user.merge(user: admin, subscription: nil, client: user), nil) }

    let(:admin) { create(:user, :admin) }
    let(:user) { create(:user, profile: :standard) }
    let(:subscription) { create(:subscription) }
    let(:client) { subscription }
    let(:model) { create(:task, subscription:) }
    let(:collaborator) { create(:collaborator, subscription:) }
    let(:pundit_user) { { user: collaborator.user, subscription:, client:, params: { id: model.id } } }
    ...
    describe '#index' do
      it 'must be successful' do
        ...
        expect(access_client_readonly).not_to receive(:exists?)
        expect(access_client_readonly.send(:index)).to be_truthy
      end
    ...
    describe '#index_archives' do
      ...
      it 'must be failure' do
        ...
        expect(access_client_readonly.send(:index_archives)).to be_falsey
      end
    end
    ...
    describe '#show' do
      it 'must be successful' do
        ...
        expect(access_client_readonly).to receive(:exists?).and_call_original
        expect(access_client_readonly.send(:show)).to be_truthy
      end
    ...
    describe '#new' do
      ...
      it 'must be failure' do
        ...
        expect(access_client_readonly.send(:new)).to be_falsey
      end
    end
    ...
    describe '#new_archives' do
      ...
      it 'must be failure' do
        ...
        expect(access_client_readonly.send(:new_archives)).to be_falsey
      end
    end
    ...
    describe '#edit' do
      ...
      it 'must be failure' do
        ...
        expect(access_client_readonly.send(:edit)).to be_falsey
      end
    end
    ...
    describe '#create' do
      ...
      it 'must be failure' do
        ...
        expect(access_client_readonly.send(:create)).to be_falsey
      end
    end
    ...
    describe '#update' do
      ...
      it 'must be failure' do
        ...
        expect(access_client_readonly.send(:update)).to be_falsey
      end
    end
    ...
    describe '#destroy' do
      ...
      it 'must be failure' do
        ...
        expect(access_client_readonly.send(:destroy)).to be_falsey
      end
    end

    describe '#exists?' do
      it 'must be exists' do
        expect(access_collaborator_readonly.send(:exists?)).to be_truthy
        expect(access_client_readonly.send(:exists?)).to be_truthy
      end
      ...
      it 'not must be exists' do
        ...
        expect(access_collaborator_readonly.send(:exists?)).to be_falsey
        expect(access_client_readonly.send(:exists?)).to be_falsey
      end
      ...
      it 'must be exists by client' do
        model.facilitator.update_columns(email: user.email)
        expect(access_user.send(:exists?)).to be_truthy

        model.facilitator.update_columns(email: user.email.reverse)
        expect(access_user.send(:exists?)).to be_falsey

        model.proprietor.update_columns(email: user.email)
        expect(access_user.send(:exists?)).to be_truthy
      end
    end
  end

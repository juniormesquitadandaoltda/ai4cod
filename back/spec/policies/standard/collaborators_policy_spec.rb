...
module STANDARD
  RSpec.describe CollaboratorsPolicy, type: :policy do
    ...
    let(:model) { create(:collaborator, subscription:, user: create(:user)) }
    ...
    describe '#new' do
      it 'must be successful' do
        expect(access_subscriber).not_to receive(:exists?)
        expect(access_subscriber.send(:new)).to be_truthy
      end
    ...
    describe '#edit' do
      it 'must be successful' do
        expect(access_subscriber).to receive(:exists?).and_call_original
        expect(access_subscriber.send(:edit)).to be_truthy
      end
    ...
    describe '#create' do
      it 'must be successful' do
        expect(access_subscriber).not_to receive(:exists?)
        expect(access_subscriber.send(:create)).to be_truthy
      end
    ...
    describe '#update' do
      it 'must be successful' do
        expect(access_subscriber).to receive(:exists?).and_call_original
        expect(access_subscriber.send(:update)).to be_truthy
      end
    ...
    describe '#destroy' do
      it 'must be successful' do
        expect(access_subscriber).to receive(:exists?).and_call_original
        expect(access_subscriber.send(:destroy)).to be_truthy
      end

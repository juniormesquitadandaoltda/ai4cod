...
module STANDARD
  RSpec.describe FacilitatorsController, type: :controller do
    ...
    let(:model) { create(:facilitator, subscription: subject.send(:current_subscription)) }
    let(:other) { create(:facilitator, subscription: create(:subscription)) }

    describe '.constants' do
      it { expect(described_class::FILTER).to eq(FacilitatorsFilter) }
    ...
    describe 'POST create' do
      let(:attributes) { attributes_for(:facilitator) }

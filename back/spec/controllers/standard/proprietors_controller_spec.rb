...
module STANDARD
  RSpec.describe ProprietorsController, type: :controller do
    render_views

    let(:model) { create(:proprietor, subscription: subject.send(:current_subscription)) }
    let(:other) { create(:proprietor, subscription: create(:subscription)) }

    describe '.constants' do
      it { expect(described_class::FILTER).to eq(ProprietorsFilter) }
    ...
    describe 'POST create' do
      let(:attributes) { attributes_for(:proprietor) }

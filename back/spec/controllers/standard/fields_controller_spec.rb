...
module STANDARD
  RSpec.describe FieldsController, type: :controller do
    ...
    let(:model) { subject.send(:current_subscription).fields.first }
    let(:other) { create(:subscription).fields.first }

    describe '.constants' do
      it { expect(described_class::FILTER).to eq(FieldsFilter) }
    ...
    describe 'PUT update' do
      ...
      it '.json unprocessable_entity' do
        put :update, params: { id: model.id, model: { values: nil } }, format: :json

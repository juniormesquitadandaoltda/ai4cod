...
module STANDARD
  RSpec.describe TasksController, type: :controller do
    ...
    let(:model) { create(:task, subscription: subject.send(:current_subscription)) }
    let(:other) { create(:task, subscription: create(:subscription)) }

    describe '.constants' do
      it { expect(described_class::FILTER).to eq(TasksFilter) }
    ...
    describe 'POST create' do
      let(:attributes) do
        ...
        attributes.merge!(
          vehicle: { id: attributes[:vehicle].reload.id },
          facilitator: { id: attributes[:facilitator].reload.id },
          proprietor: { id: attributes[:proprietor].reload.id }
        )
        attributes
      end
    ...
    describe '#set_model!' do
      it 'must find' do
        ...
        subject.instance_variable_set('@current_client', create(:user, email: model.facilitator.email))
        expect(subject.send(:set_model!)).to eq(model)
        expect(assigns(:model)).to be_a(described_class::MODEL)

        subject.instance_variable_set('@current_client', create(:user, email: model.proprietor.email))
        expect(subject.send(:set_model!)).to eq(model)
        expect(assigns(:model)).to be_a(described_class::MODEL)
      end
      ...
      it 'not must find' do
        ...
        subject.instance_variable_set('@current_client', subject.current_user)
        expect { subject.send(:set_model!) }.to raise_exception(::ActiveRecord::RecordNotFound)
      end
    end

...
module STANDARD
  RSpec.describe WebhooksController, type: :controller do
    ...
    let(:model) { create(:webhook, subscription: subject.send(:current_subscription)) }
    let(:other) { create(:webhook, subscription: create(:subscription)) }

    describe '.constants' do
      it { expect(described_class::FILTER).to eq(WebhooksFilter) }
    ...
    describe 'POST create' do
      let(:attributes) { attributes_for(:webhook) }
    ...
    describe 'PUT update' do
      ...
      it '.json unprocessable_entity' do
        put :update, params: { id: model.id, model: { url: nil } }, format: :json
    ...
    it '#model_params' do
      attributes = { actived: '', url: '', resource: '', event: '' }

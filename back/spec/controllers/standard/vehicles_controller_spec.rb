...
module STANDARD
  RSpec.describe VehiclesController, type: :controller do
    ...
    let(:model) { create(:vehicle, subscription: subject.send(:current_subscription)) }
    let(:other) { create(:vehicle, subscription: create(:subscription)) }

    describe '.constants' do
      it { expect(described_class::FILTER).to eq(VehiclesFilter) }
    ...
    describe 'PUT update' do
      ...
      it '.json unprocessable_entity' do
        put :update, params: { id: model.id, model: { chassis: nil } }, format: :json

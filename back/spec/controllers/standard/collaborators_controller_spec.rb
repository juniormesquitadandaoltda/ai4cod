...
module STANDARD
  RSpec.describe CollaboratorsController, type: :controller do
    ...
    let(:model) { create(:collaborator, subscription: subject.send(:current_subscription)) }
    let(:other) { create(:collaborator, subscription: create(:subscription)) }

    describe '.constants' do
      it { expect(described_class::FILTER).to eq(CollaboratorsFilter) }
    ...
    describe 'POST create' do
      let(:attributes) do
        attributes = attributes_for(:collaborator)
        attributes.merge!(
    ...
    describe 'PUT update' do
      ...
      it '.json unprocessable_entity' do
        put :update, params: { id: model.id, model: { user: nil } }, format: :json

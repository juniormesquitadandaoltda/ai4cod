...
module STANDARD
  RSpec.describe ArchivesController, type: :controller do
    ...
    let(:model) { create(:archive, subscription: subject.send(:current_subscription)) }
    let(:other) { create(:archive, record: task) }
    let(:task) { create(:task, subscription: create(:subscription)) }
    ...
    describe 'POST create' do
      let(:attributes) do
        { filename: 'name.txt', base64: 'data', record: { id: model.record_id, type: model.record_type } }
      end
    ...
    describe 'PUT update' do
      ...
      it '.json unprocessable_entity' do
        put :update, params: { id: model.id, model: { filename: nil } }, format: :json
    ...
    it '#create_params' do
      attributes = { record: { id: '', type: '' }, filename: '', base64: '' }

      subject.params = { model: attributes }

      expect(subject.send(:create_params).as_json).to eq(attributes.merge(
    ...
    it '#update_params' do
      ...
      expect(subject.send(:update_params).as_json).to eq(attributes.as_json)
    end
  end
end

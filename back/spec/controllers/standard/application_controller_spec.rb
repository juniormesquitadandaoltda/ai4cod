...
module STANDARD
  RSpec.describe ApplicationController, type: :controller do
    ...
    it '#current_subscription' do
      subscription = create(:subscription)
      subject.instance_variable_set('@current_subscription', subscription)

      expect(subject.send(:current_subscription)).to eq(subscription)
    end

    it '#current_client' do
      user = create(:user)
      subject.instance_variable_set('@current_client', user)

      expect(subject.send(:current_client)).to eq(user)
    end
    ...
    it '#pundit_user' do
      ...
      expect(subject.send(:pundit_user)).to eq(
        ...
        subscription: subject.current_subscription,
        client: subject.current_client,
        params: { id: 0 }
      )
    end

    it '#authorize_user!' do
      expect(subject).to receive(:controller_path).and_return('controller').at_least(:once)
      expect(subject).to receive(:action_name).and_return('action').at_least(:once)

      expect(subject).to receive(:authorize).with(:controller, :action).at_least(:once)

      subject.send(:authorize_user!)
      expect(subject.current_client).to eq(subject.current_subscription)
    end
